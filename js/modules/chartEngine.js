...
export function renderStudioChart(ctx, record) {
  ...
  if (document.getElementById('studioReverseSortOrder')?.getAttribute('aria-pressed') === 'true') chartRows.reverse();
  ...
  const reverseValueAxis = document.getElementById('studioReverseValueAxis')?.getAttribute('aria-pressed') === 'true';
  ...
  ctx.state.studioChartConfig = { orientation: horizontal ? 'horizontal' : 'vertical', valueAxisReversed: reverseValueAxis, rankSemantic, rankValueMin, rankValueMax, valueAxisMin: axisMin, valueAxisMax: yMax, labels: fullLabels.slice() };
