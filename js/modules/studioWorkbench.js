...
export function initStudioWorkbench(ctx) {
  ...
  ctx.api.saveStudioData = async approve => {
    ...
    if (chart) { const options = chart.getOption(); const series = options.series?.[0] || {}; const horizontal = ctx.state.studioChartConfig?.orientation === 'horizontal'; const axisLabels = horizontal ? options.yAxis?.[0]?.data : options.xAxis?.[0]?.data; const fallbackAxisLabels = horizontal ? options.xAxis?.[0]?.data : options.yAxis?.[0]?.data; const sourceLabels = Array.isArray(ctx.state.studioChartConfig?.labels) ? ctx.state.studioChartConfig.labels : null; const points = (series.data || []).map((point, index) => ({ label: sourceLabels?.[index] ?? axisLabels?.[index] ?? fallbackAxisLabels?.[index] ?? (typeof point === 'object' ? point.name : undefined), value: typeof point === 'object' ? point.rawValue ?? point.value : point })); savedChart = { record_id: record.id, title: $('studioChartTitleInput')?.value || 'Observatory Draft', chart_type: $('studioChartTypeSelect')?.value || 'bar', orientation: ctx.state.studioChartConfig?.orientation || 'vertical', rankSemantic: ctx.state.studioChartConfig?.rankSemantic === true, rankValueMin: ctx.state.studioChartConfig?.rankValueMin, rankValueMax: ctx.state.studioChartConfig?.rankValueMax, valueAxisReversed: ctx.state.studioChartConfig?.valueAxisReversed === true, valueAxisMin: ctx.state.studioChartConfig?.valueAxisMin, valueAxisMax: ctx.state.studioChartConfig?.valueAxisMax, labels: points.map(point => point.label), values_data: points.map(point => point.value) }; }
